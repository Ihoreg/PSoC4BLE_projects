...
CYBLE_API_RESULT_T CyBle_Start(CYBLE_CALLBACK_T callbackFunc)
{
    ...
    if(callbackFunc != NULL)
    {
        ...
        if(cyBle_stackMemoryRam == NULL)
        {
            ...
            {
                (void)memset(cyBle_stackMemoryRam, 0, CYBLE_STACK_RAM_SIZE);
            }
        }
    #endif /* CYBLE_SHARING_MODE_EXPORT */
        ...
        if(apiResult == CYBLE_ERROR_OK)
        {
            ...
            apiResult = CyBle_StackInit(&CyBle_EventHandler, cyBle_stackMemoryRam, 
                CYBLE_STACK_RAM_SIZE - CYBLE_GATT_PREPARE_WRITE_BUFF_LEN, 
                cyBle_stackDataBuff, CYBLE_STACK_APP_MIN_POOL, cyBle_flashStorage.stackFlashptr, CYBLE_STACK_FLASH_SIZE);
        #else
...
CYBLE_API_RESULT_T CyBle_GapRemoveBondedDevice(CYBLE_GAP_BD_ADDR_T* bdAddr)
{
    ...
    if(cyBle_pendingFlashWrite == 0u)
    {
        ...
        else
        {
            ...
            apiResult = CyBle_GapGetPeerBdHandle(&bdHandle, bdAddr);
            if(apiResult == CYBLE_ERROR_OK)
            {
                ...
                cyBle_pendingFlashClearCccdHandle = bdHandle;
            }
        }
        if(apiResult == CYBLE_ERROR_OK)
        {
